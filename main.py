...
def get_text_messages(message):
    ...
    if message.text == "/start" and global_bots == 1:
        ...
        while True:
            ...
            #bot.send_message(message.chat.id, str(now.hour))
            i +=1
            if (int(now.hour) + 3)% 24 == 1 and int(now.minute) %2 == 0 and int(now.second) < 20 and cnt_wake_up_0 == 0:
                print("OKAY")
                for _ in range(TIMES_WAKE_UP):
                    bot.send_message(message.chat.id, "РОТА ПОДЪЕМ!")
                cnt_wake_up_0 = 1
                cnt_wake_up_1 = 0
            if (int(now.hour) + 3 )% 24 == 1 and int(now.minute) %2 == 1 and int(now.second) < 20 and cnt_wake_up_0 == 1:
                print("OKAY1")
                for _ in range(TIMES_WAKE_UP):
